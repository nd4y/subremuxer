...
этот формат не читает, импортирует ноль серверов. Например, при подмене на Happ панель вернёт
массив Xray-JSON, и клиент вроде NekoBox, понимающий только base64, останется с пустым списком.
...
Railway отклоняет Dockerfile с инструкцией `VOLUME` — сборка падает до начала с ошибкой
...
Удаление профиля не спрашивает подтверждения: вместо диалога появляется снекбар с кнопкой
«Отменить» и круговым обратным отсчётом. За пределами этого окна профиль остаётся восстановимым
ещё сутки.
...
* Адрес исходной подписки задаёт администратор, и он ничем не ограничен — панель часто живёт
  во внутренней сети. Не давайте доступ к админке тем, кому не доверяете.
